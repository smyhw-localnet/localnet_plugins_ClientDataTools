...
public class lnp 
{
	...
	public static void cmd(Client_sl User,String cmd)
	{
		...
		{
			if(CommandFJ.js(cmd)<4) {User.sendto("参数不足...");}
			User.sendto("======ClientDataTools======");
			User.sendto("指令例如:cdt temp <终端ID> <下列指令> #读取Temp信息");
			User.sendto("指令例如:cdt bin <下列指令> #读取持久性信息");
			User.sendto("show #查看目标终端的所有ClientData");
			User.sendto("del <plugin_ID> <key> #删除指定数据");
			User.sendto("add <plugin_ID> <key> <value> #添加指定数据");
			return;
		}
		...
		if(DoClient==null) 
		{User.sendto("终端<"+CommandFJ.fj(cmd, 2)+"不存在");return;}
		...
		if(DoMap==null) {User.sendto("类型只能为<bin>或<temp>,而你的输入为<"+CommandFJ.fj(cmd, 1)+">");return;}
		...
		case "show":
		{
			...
			User.sendto("终端实例<"+CommandFJ.fj(cmd, 2)+">的ClientData列表");
			while (iter.hasNext()) 
			{
				Map.Entry entry = (Map.Entry) iter.next();
				String key = (String) entry.getKey();
				Object val = entry.getValue();
				User.sendto("KEY=<"+key+">;VALUE=<"+val.toString()+">");
			}
			break;
		}
		case "del":
		{
			if(CommandFJ.js(cmd)<6) {User.sendto("参数不足");return;}
			...
			User.sendto("成功将终端<"+CommandFJ.fj(cmd, 2)+">的ClientData<"+PluginID+"><"+KEY+">删除");
			break;
		}
		case "add":
		{
			if(CommandFJ.js(cmd)<7) {User.sendto("参数不足");return;}
			String PluginID = CommandFJ.fj(cmd, 4);
			String KEY = CommandFJ.fj(cmd, 5);
			...
			User.sendto("成功将终端<"+CommandFJ.fj(cmd, 2)+">的ClientData<"+PluginID+"><"+KEY+">设置为<"+VALUE+">");
			break;
		}
		default:
		{
			User.sendto("未知的选项<"+gg+">");
		}
		}
		return;
	}
